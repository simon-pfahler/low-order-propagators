...
relerrs = torch.sqrt(variances) / means

max_relerr_idx = torch.argmax(relerrs)

print(f"Maximum relative error observed for {filenames[max_relerr_idx]}")
print(f"This file has a relative error of {relerrs[max_relerr_idx]}")
print(f"The average relative error across all calculations is {relerrs.mean()}")
